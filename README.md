...
- 路线前展示适用版本、核验方法和日期、前置结局、周目与存档说明；资料整理不会自动标为实机验证。
- 树内搜索日期（支持 12/24、12月24日）及选项文字，跳转高亮；阅读书签与实际选择分开，随备份保存。
- 记录模式可折叠已记录步骤、定位当前选择。
- 每一步可展开来源位置；白二争议节点显示补充证据、版本边界和待核验状态。
- 简短前置说明与直接看步骤入口；提供经核对的 Coda / ATRI 多结局存档规划。
- 36 条公开路线独立链接、静态正文、页面元信息和站点地图；旧链接仍可使用。[搜索与分享](docs/search-and-sharing.md)。
- [内容维护流程与待核验清单](docs/content-maintenance.md)。
- 选定路线后使用加宽、不透底的阅读区域，淡化并暂停装饰背景；返回游戏库恢复原布局。
- 路线及每个步骤均有纠错入口，预填攻略信息后跳转 GitHub Issues，由用户自行提交公开反馈；不附带本机游玩记录。
