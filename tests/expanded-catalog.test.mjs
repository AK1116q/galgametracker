...
import { createSession, recordChoice, replay, skipChoice } from '../core/engine.mjs';
import { validatePack } from '../core/validation.mjs';
const dir = new URL('../data/catalog/',import.meta.url);
const packs = readdirSync(dir).map(f=>JSON.parse(readFileSync(new URL(f,dir),'utf8')));
...
test('five researched works cover 26 terminating paths without claiming playtests',()=>{
  ...
  assert.equal(packs.reduce((n,p)=>n+p.routes.length,0),26);
  for (const p of packs) {
    ...
    for(const r of p.routes) {
      ...
      if(r.requiredEndingIds.length) assert.throws(()=>createSession(p,r.id,[]),/前置/);
    }
  }
});
test('conditional branches and title-menu endings retain their prerequisites',()=>{
  ...
  assert.deepEqual(atri.routes.find(r=>r.id==='true').requiredEndingIds,['good-end','bad-end']);
  ...
  assert.deepEqual(sabbat.routes.find(r=>r.id==='nene-restart').requiredEndingIds,['nene-end']);
  const riddle=pack('riddle-joker');
  let s=createSession(riddle,'nanami',[]);
  const path=guidePath(riddle,'nanami');
  for(const node of path.nodes) s=node.choice.skipTo ? skipChoice(riddle,s) : recordChoice(riddle,s,node.optionId);
  assert.deepEqual(replay(riddle,s).position,{kind:'ending',id:'nanami-end'});
  assert.equal(riddle.routes.find(r=>r.id==='normal').requiredEndingIds.length,1);
});
