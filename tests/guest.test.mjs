...
import { emptyLibrary, validateBackup, mergeBackup, packKey } from '../core/backup.mjs';
...
test('legacy backups remain readable and saved routes survive export/merge', () => {
  ...
  const item = { packKey: packKey(pack), routeId: pack.routes[0].id, at: '2026-09-18T00:00:00Z' };
  ...
  assert.throws(() => validateBackup({ ...incoming, favorites: [{ ...item, routeId: 'missing' }] }, [pack]));
  assert.throws(() => validateBackup({ ...incoming, favorites: [item, item] }, [pack]));
});
