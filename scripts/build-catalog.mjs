...
const D = (locator, prompt, options, extra = {}) => ({
  locator,
  prompt,
  options,
  ...extra,
...
const permission =
  "仅整理分支、前置和结局事实；中文提示自行概述，不转载攻略正文、图片或剧情。";
function build(
  id,
  title,
  aliases,
  ...
  const pack = {
    ...
    game: { id, title, aliases },
    release: {
      id: `${id}-pc`,
      label: "PC / Steam 主线 · 中文含义提示",
      locale: "zh-CN",
    },
    notes: [
      "从新游戏开始按目标路径连续选择；不覆盖 AFTER、差分 CG 或全成就。",
      "提示为中文概述，不是汉化补丁逐字原文；未核实选项排列顺序的节点不显示第几项。",
      "已按所列来源核对分支与前置，未实机通关；偏离路径后的结果不作推断。",
      ...notes,
    ],
    sources: sources.map((reference, i) => ({
      id: `source-${i + 1}`,
  ...
  for (const [key, label] of Object.entries(prerequisites))
    pack.endings.push({
      id: key,
      safeLabel: label,
      hiddenTitle: label,
  ...
  for (const route of routes) {
    ...
    pack.routes.push({
      id: rid,
      safeLabel: label,
      entryChoiceId: `${rid}-01`,
      requiredEndingIds: required,
    });
    pack.endings.push({
      id: `${rid}-end`,
      safeLabel: label,
    ...
    steps.forEach(([key, selected], i) => {
      const def = definitions[key],
        cid = `${rid}-${String(i + 1).padStart(2, "0")}`;
      const next =
        i + 1 === steps.length
          ? { kind: "ending", id: `${rid}-end` }
          : { kind: "choice", id: `${rid}-${String(i + 2).padStart(2, "0")}` };
      const node = {
        id: cid,
        locator: def.locator,
        prompt: def.prompt,
        ...
        ...(def.options.length === 1 && def.kind !== "instruction"
          ? { optionsComplete: false }
          : {}),
        options: def.options.map((text, j) => ({
          id: `option-${j + 1}`,
          text,
          next: j === selected ? next : { kind: "unknown", id: "off-path" },
        })),
        rules: [
          {
            routeId: rid,
            when: { all: [] },
            recommendedOptionIds: [`option-${selected + 1}`],
            sourceId: "source-1",
            sourceLocator: `${label} / ${def.locator} / ${key}；与来源 2 对照`,
          },
        ],
      };
  ...
  pack.reviews.push({
    reviewer: "资料交叉核对（非实机）",
    date: "2026-09-18",
    routeIds: pack.routes.map((r) => r.id),
    method: "source_crosscheck",
    evidence:
      "逐项对照所列资料的主线选法、条件节点及解锁前置。差异处理见 notes 与 docs/catalog-research.md。",
  });
  writeFileSync(
    new URL(`${id}.route.json`, root),
    JSON.stringify(pack, null, 2) + "\n",
  );
}
const senren = {
  truth: D("共通线 · 回答问题", "如何回答", ["如实说明", "含糊带过"]),
  city: D("共通线 · 城市与乡村", "对居住环境的看法", [
    "暂时说不准",
    "更喜欢城市",
  ]),
...
build(
  ...
  [
    ...
    [
      "koharu",
      "鞍马小春",
      [...fish, S("trust", 0), S("reassure", 1), S("sub", 0)],
      ["main-completed"],
    ],
    [
      "roka",
      "马庭芦花",
      [...fish, S("trust", 0), S("reassure", 1), S("sub", 1)],
      ["main-completed"],
    ],
    ["normal", "通常结局", [...fish, S("trust", 1), S("reassure", 1)]],
  ],
  ["小春、芦花需先完成任意一位主角路线；采野菜或独自行动后没有钓鱼分支选项。"],
  { "main-completed": "芳乃、茉子、丛雨、蕾娜中任意一条路线已通关" },
);

const riddle = {
  test: D("共通线 · 实验", "接下来的实验", ["继续与茉优实验", "邀请七海"]),
  swim: D("共通线 · 泳装", "关注谁", ["二条院", "七海"]),
  ask: D("追加选项 · 七海分支", "是否追问话题", ["询问下去", "不提这个话题"], {
    optional: true,
  }),
...
build(
  ...
  [
    [
      "ayase",
      "三司绫濑",
      [...rbase, S("advice", 0), S("dorm", 0), S("who", 0)],
    ],
    [
      "nanami",
      "在原七海",
    ...
    [
      "hazuki",
      "二条院羽月",
      [...rbase, S("advice", 0), S("dorm", 0), S("who", 1)],
    ],
    ["mayu", "式部茉优", [...rbase, S("advice", 1), S("who", 0)]],
    [
      "chisaki",
      "壬生千咲",
      [...rchi, S("advice", 1), S("who", 2)],
      ["main-completed"],
    ],
    [
      "normal",
      "通常结局（二周目选法）",
      [...rchi, S("advice", 0), S("dorm", 1), S("who", 1)],
      ["main-completed"],
    ],
  ],
  [
    ...
    "部分转载称千咲需四线全通；原日文攻略和 Steam Chuee 均明确为任意一线，采用后者一致的条件。",
  ],
  { "main-completed": "绫濑、七海、羽月、茉优中任意一条路线已通关" },
);
...
const sabbat = {
  ...
  hand: D("共通线 · 握手", "是否再请求一次", [
    "请她再握一次",
    "不好意思说出口",
  ]),
  look: D("共通线 · 目光", "如何回应", ["移开视线", "称赞她"]),
  final: D("共通线 · 最后选择", "此时想到的人或事", [
    "宁宁",
    "爱瑠",
    "憧子学姐",
    "乐队",
...
build(
  "sabbat-of-the-witch",
  "魔女的夜宴",
  ["Sabbat of the Witch", "サノバウィッチ", "魔宴"],
  [
  ...
  [
    ...
    "和奏需任意主角路线通关；若先攻略宁宁，需要读完 RESTART 后半篇。",
  ],
  { "main-completed": "爱瑠、䌷、憧子或宁宁后半篇中任意一条已通关" },
);

const atri = {
  keep: D("7月18日 · 市场", "凯瑟琳提出带走亚托莉", [
    "交给凯瑟琳",
    "让亚托莉留在自己这里",
  ]),
